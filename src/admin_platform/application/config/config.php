...
if(ENVIRONMENT == 'development' &&  strpos($_SERVER['HTTP_HOST'], 'localhost') !== false ){
  $config['base_url'] = "http://" . $_SERVER['HTTP_HOST'] . "/admin_console";
}else{
  $config['base_url'] = "https://" . $_SERVER['HTTP_HOST'] . "/admin_console";
}
...
|
*/
$config['time_reference'] = 'US/Eastern';
...
spl_autoload_register(function ($class) {
    ...
    if (substr($class, 0, 3) !== 'CI_') {
        if (file_exists($file = dirname(FCPATH).'/common/libraries/' . $class . '.php')) {
            include $file;
        }
    }

    $subdir = glob(APPPATH . 'libraries/*', GLOB_ONLYDIR);
    ...
    $subdir = glob(dirname(FCPATH).'/common/libraries/*', GLOB_ONLYDIR);
    foreach ($subdir as $dir) {
        if (file_exists($file = $dir ."/". $class . '.php')) {
            include $file;
        }
...
if (file_exists(APPPATH . '../.env')) {
    $variables = parse_ini_file(APPPATH . '../.env');
